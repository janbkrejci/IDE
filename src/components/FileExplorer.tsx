...
const FileItem: React.FC<FileItemProps> = ({
  ...
  return (
    <>
      <div
        className={`group flex items-center h-7 cursor-pointer rounded ${
          activeFile?.path === file.path ? 'bg-[#37373d]' : 'hover:bg-[#2a2a2a]'
      ...
      {contextMenu && (
        <ContextMenu
          ...
          onCreateFile={() => onCreateItem(file.path, 'file')}
          onCreateFolder={() => onCreateItem(file.path, 'directory')}
        />
      )}
    ...
    const renderFiles = (parentFiles: FileInfo[], level: number) => {
      return sortFiles(parentFiles).map((file) => (
        <FileItem
          ...
          onRename={renameItem}
          onDelete={deleteItem}
          onCreateItem={(parentPath, type) =>
            setNewItemState({ type, parentPath, name: '' })
          }
          onMove={handleMove}
          ...
          {newItemState?.parentPath === file.path && (
            <div className="h-7 flex items-center" style={{ paddingLeft: `${(level + 2) * 12}px` }}>
              <input
                ...
                placeholder={`New ${newItemState.type}...`}
